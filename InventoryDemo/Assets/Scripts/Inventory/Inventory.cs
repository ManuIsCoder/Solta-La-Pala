...
public class Inventory : MonoBehaviour, IItemContainer
{
    ...
    {
        // El inventario esta lleno, no se a�ade
        ...
        {
            // No se apila y se a�ade a un slot
            if (_items[slot] != null)
            {
                ...
                _itemIcons[_index].enabled = true; // Agregado: el original no lo prendia
            ...
            _itemIcons[slot].enabled = true; // Agregado: el original no lo prendia
            if (quantity > 1)
                _itemIconQuantities[slot].text = quantity.ToString();
            else
                _itemIconQuantities[slot].text = "";

            // Una vez a�adido, el _index se va al primer hueco que encuentre libre
            _index = 0;
    ...
    private void RemoveAt(int slot)
    {
        ...
        _itemIcons[slot].enabled = false; // Agregado: el original no lo apagaba
        _itemIconQuantities[slot].text = "";
        _index = slot;
    }
