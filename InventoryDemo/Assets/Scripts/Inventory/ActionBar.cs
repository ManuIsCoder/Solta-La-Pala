...
public class ActionBar : MonoBehaviour, IItemContainer
{
    ...
    private int _selectedSlot = -1; // Default = -1 (Nota: representa el index del array, no el n�mero de bot�n)
    ...
    void Update()
    {
        // Cambiado: el original mapeaba tecla 1 -> indice 1 (salteando el
        // primer slot, indice 0, que quedaba sin tecla). Ahora es 1:1.
        if (_inputs.Equip1) SelectSlot(0);
        else if (_inputs.Equip2) SelectSlot(1);
        else if (_inputs.Equip3) SelectSlot(2);
    }
    ...
    private void SelectSlot(int slot)
    {
        ...
        {
            // Agregado: el original no chequeaba si el slot estaba vacio y explotaba con NullReferenceException
